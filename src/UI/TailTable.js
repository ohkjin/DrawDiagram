...
export default function TailTable({key,rank,movieNm,salesAcc,audiCnt,rankInten}) {
  return (
    <tr key = {key} className="hover:bg-orange-300 hover:text-blue-400 hover:font-extrabold">
        ...
        <td className="py-4 px-6 text-sm  text-gray-900 whitespace-nowrap dark:text-white">
            {(parseInt(rankInten)>0)?
            <span className='text-red-600'>▲{rankInten}</span>
            : (parseInt(rankInten)<0)?
            <span className='text-blue-600'>▼{rankInten}</span>
